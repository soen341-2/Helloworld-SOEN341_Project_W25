...
import { Firestore, collection, doc, updateDoc, getDocs, getDoc, setDoc } from '@angular/fire/firestore';
...
channels: any[] = [];
  ...
  async createPublicChannel(title: string) {
    ...
    const channelRef = doc(this.firestore, 'channels', title.replace(/\s+/g, '-').toLowerCase());
  ...
  async requestToJoin(channelId: string) {
    ...
    const requestRef = doc(this.firestore, 'joinRequests', `${this.currentUser.id}_${channelId}`);
  ...
  async leaveChannel(channelId: string) {
    ...
    if (channelSnap.exists()) {
      ...
      allowedUsers = allowedUsers.filter((userId: any) => userId !== this.currentUser.id);
