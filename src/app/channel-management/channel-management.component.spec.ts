...
describe('ChannelManagementComponent', () => {
  ...
  it('should load channels from Firestore', async () => {
    ...
    await setDoc(doc(channelsCollection, 'general'), {
      title: 'General',
      isPrivate: false,
    });
  ...
  it('should load join requests from Firestore', async () => {
    ...
    await setDoc(doc(joinCollection, 'user123_channel123'), {
      userId: 'user123',
    ...
    const found = component.joinRequests.find(
      (req) => req.channelId === 'channel123'
    );
    expect(found).toBeTruthy();
  });
