...
describe('ChannelAreaComponent', () => {
  ...
  it('should return replied message content', () => {
    component.messages = [
      { id: 'msg1', sender: 'tester', message: 'first', timestamp: '', replyId: null }
    ];
    expect(component.getRepliedMessageContent('msg1')).toBe('first');
    expect(component.getRepliedMessageContent('unknown')).toBe('Deleted message');
  });
  ...
  it('should do nothing when user deletes message', () => {
    ...
    expect(console.log).not.toHaveBeenCalledWith("Message deleted successfully");
  });
  ...
  it('should alert on missing invite info', async () => {
    ...
    expect(window.alert).toHaveBeenCalledWith("Please select a user to invite.");
  });
